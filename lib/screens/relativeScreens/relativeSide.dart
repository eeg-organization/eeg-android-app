import 'package:adv_eeg/controllers/relative/detailedRelativeViewController.dart';
...
class RelativeSide extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(
      () => ModalProgressHUD(
        inAsyncCall: relativeController.isLoading.value,
        child: Scaffold(
          body: Obx(
            () => !relativeController.isLoading.value
                ? ListView.builder(
                    itemCount:
                        relativeController.data.value.message!.patient!.length,
                    itemBuilder: (context, index) => ListTile(
                      onTap: () {
                        Get.to(
                            () => PatientDetailedView(
                                  uid: relativeController
                                      .data.value.message!.patient![index].uid!,
                                ),
                            arguments: relativeController
                                .data.value.message!.patient![index]);
                      },
                      title: Text(
                          '${relativeController.data.value.message!.patient![index].name}'),
                      subtitle: Text(
                          '${relativeController.data.value.message!.patient![index].email}'),
                    ),
                  )
...
class PatientDetailedView extends StatelessWidget {
  PatientDetailedView({super.key, required this.uid});
  String uid;

  @override
  Widget build(BuildContext context) {
    RelativeDetailedController relativeController =
        Get.put(RelativeDetailedController(data: uid));
    return Obx(
      () => ModalProgressHUD(
        inAsyncCall: relativeController.isLoading.value,
        child: Scaffold(
